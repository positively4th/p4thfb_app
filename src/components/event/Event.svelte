<script>
  ...
  $: visibleRelatedEvents = R.pipe(
    R.values,
    R.unnest,
    R.filter((re) => re && visibleRelatedEventIds.includes(re.__id))
  )(relatedEvents);
  ...
  $: onOffKeyLabelPairs = ((relatedEvents) => {
    const { idTagsMap, idEventMap } = R.reduce((res, [tag, events]) => {
      return R.reduce((res, event) => {
        const id = event.__id;
        res.idTagsMap[id] = (res.idTagsMap[id] || []).concat([
          tagTools.getName(tag),
        ]);
        res.idEventMap[id] = event;
        return res;
      }, res)(events);
    })({ idTagsMap: {}, idEventMap: {} }, R.toPairs(relatedEvents));
    const res = R.map(([id, event]) => {
      return {
        key: id,
        label: asEvent(event).shortTitle((idTagsMap[id] || []).join(", ")),
      };
